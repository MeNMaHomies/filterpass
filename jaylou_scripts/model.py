...
class SAPClassifier(nn.Module):
    def __init__(self, model_name="facebook/wav2vec2-base", revision=None, freeze_extractor=True):
        """
        Wav2Vec2 with multi-head attention pooling and a deeper classifier head.

        Args:
            model_name (str):        HuggingFace model repo to load.
            revision (str):          Specific git commit/revision of the model.
            freeze_extractor (bool): If True, freezes the entire encoder initially.
        """
        super().__init__()

        print(f"Initialising SAP Classifier (Revision: {revision or 'Latest'})")
